...
function registrarEventos() {
    ...
    // Botones
    frmaltaproducto.btnAltaCategoria.addEventListener("click",altaProducto);
...
async function cargarDesplegables() {
  ...
  if (respuesta.ok) {
    ...
    frmaltaproducto.id_season.innerHTML = options;
...
async function altaProducto() {
  let nombre = frmaltaproducto.category_name.value.trim();
  let descripcion = frmaltaproducto.description.value.trim();
  let id_season = frmaltaproducto.id_season.value.trim();
  let seasonalAvailable = frmaltaproducto.seasonal_product_available.checked ? 1 : 0;
  let likes = parseInt(frmaltaproducto.likes.value);
